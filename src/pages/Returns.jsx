...
const Returns = () => {
  ...
  const fetchReturns = async () => {
    try {
      ...
      const filtered = data.filter(order => order.status === 'Return Requested' || order.status === 'Returned' || order.status === 'Return Rejected');
      setReturns(filtered);
    } catch (error) {
    ...
    <div className="container">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '40px 0', marginBottom: '40px' }}>
        <div style={{ color: 'var(--text-secondary)', fontSize: '14px' }}>
          Home / <span style={{ color: 'var(--text-primary)', fontWeight: '500' }}>My Returns</span>
        </div>
      ...
      <div style={{ display: 'flex', gap: '100px', marginBottom: '140px' }}>
        ...
        <aside style={{ width: '200px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            <Link to="/returns" style={{ color: 'var(--primary)', marginLeft: '16px', textDecoration: 'none' }}>My Returns</Link>
        ...
        <div style={{ flex: 1, padding: '40px 80px', boxShadow: '0px 1px 13px rgba(0, 0, 0, 0.05)', borderRadius: '4px' }}>
          <h2 style={{ fontSize: '20px', fontWeight: '500', color: 'var(--primary)', marginBottom: '24px' }}>My Returns</h2>
          
          {loading ? (
            <Loader type="spinner" />
          ) : returns.length === 0 ? (
            <p style={{ color: 'var(--text-secondary)' }}>You have no returns at this time.</p>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
              {returns.map(order => {
                const currentStatus = order.status || 'Return Requested';
                
                let statusColor = '#f97316'; // Orange for Return Requested
                if (currentStatus === 'Returned') statusColor = '#6b7280'; // Gray
                else if (currentStatus === 'Return Rejected') statusColor = '#ef4444'; // Red
